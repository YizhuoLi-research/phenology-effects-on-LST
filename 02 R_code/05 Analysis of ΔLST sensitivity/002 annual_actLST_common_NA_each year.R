...
# 创建输出文件夹（如果不存在）
new_folder <- "./NA_Results/0.actLSTmean_yr/0common_pixel/"
dir.create(new_folder, showWarnings = FALSE)

# 循环处理2013到2021年
for (year in 2013:2021) {
  # 读取 actLSTmean 文件
  actLSTmean <- rast(paste0("./NA_Results/0.actLSTmean_yr/", year, "_actLSTmean.tif"))
...
# ####### 1 读取 ####
# actLSTmean <- rast("./NA_Results/0actLSTmean_yr/2013_actLSTmean.tif")
# 
# 
# ###### 2  取共diff和sum共有的像元####
# aa <- rast("./NA_Results/0diff_result/0common_pixel/average_diff_1_2013.tif")  #去找与diff共同像元
# sample_diff = aa
# sample_diff[is.finite(sample_diff)] = 1
# 
...
# 
# new_folder <- "./NA_Results/0actLSTmean_yr/0common_pixel/"
# dir.create(new_folder, showWarnings = FALSE)
# 
# writeRaster(actLSTmean,"./NA_Results/0actLSTmean_yr/0common_pixel/2013_actLSTmean.tif",overwrite=T)
