...
# # 结论：noen站点与普通站点的LST日变化曲线一致，用的都是local time
# # 因此  data1的时间不用转换时区

# #####################################   对LST数据noen站点数据UTC不用进行时区转换的测验   ########################################
# 
# #对data1_DB
# # ###################################     非 NOEN站点单日折线图测试
# 
# # 普通 local time 站点1
# filtered_data <- data1_DB %>%
#   filter(SiteID == "US-UMB" & substr(TIMESTAMP_START, 1, 8) == "20180610")
# # 查看筛选后的结果
# head(filtered_data)
# library(ggplot2)
# # 提取 TIMESTAMP_START 的后四位作为时间部分
# filtered_data$time_part <- substr(filtered_data$TIMESTAMP_START, nchar(filtered_data$TIMESTAMP_START) - 3, nchar(filtered_data$TIMESTAMP_START))
# # 生成折线图
# ggplot(filtered_data, aes(x = time_part, y = corrected_surfaceTemp_97, group = 1)) +
#   geom_line() + 
#   geom_point() +  # 可选：添加点
#   labs(x = "Time (HHMM)", y = "Corrected Surface Temp 97", 
#        title = "LST 97 vs Time") +
#   theme_minimal() +
...
# ######### NOEN-UTC 站点１
# 
# filtered_data2 <- data1_DB %>%
#   filter(SiteID == "US-xLE" & substr(TIMESTAMP_START, 1, 8) == "20180610")
# # 查看筛选后的结果
# head(filtered_data2)
# library(ggplot2)
# # 提取 TIMESTAMP_START 的后四位作为时间部分
# filtered_data2$time_part <- substr(filtered_data2$TIMESTAMP_START, nchar(filtered_data2$TIMESTAMP_START) - 3, nchar(filtered_data2$TIMESTAMP_START))
# # 生成折线图
# ggplot(filtered_data2, aes(x = time_part, y = tau_corrected_surfaceTemp_97, group = 1)) +
#   geom_line() + 
#   geom_point() +  # 可选：添加点
#   labs(x = "Time (HHMM)", y = "Tau Corrected Surface Temp 97", 
#        title = "Tau_LST_97 vs Time (US-xLE,20180610)_NOEN") +
#   theme_minimal() +
#   theme(axis.text.x = element_text(angle = 45, hjust = 1))
# ###data2
# aa <- data2_DB_DOY %>%
#   filter(SiteID == "US-xLE" )
# 
# 
# 
# ######### NOEN-UTC 站点２
# 
# 
# filtered_data3 <- data1_DB %>%
#   filter(SiteID == "US-xCL" & substr(TIMESTAMP_START, 1, 8) == "20180610")
# # 查看筛选后的结果
# head(filtered_data3)
# library(ggplot2)
# # 提取 TIMESTAMP_START 的后四位作为时间部分
# filtered_data3$time_part <- substr(filtered_data3$TIMESTAMP_START, nchar(filtered_data3$TIMESTAMP_START) - 3, nchar(filtered_data3$TIMESTAMP_START))
# # 生成折线图
# ggplot(filtered_data3, aes(x = time_part, y = tau_corrected_surfaceTemp_97, group = 1)) +
#   geom_line() + 
#   geom_point() +  # 可选：添加点
#   labs(x = "Time (HHMM)", y = "Tau Corrected Surface Temp 97", 
#        title = "Tau_LST_97 vs Time(US-xCL,20180610)_NOEN") +
#   theme_minimal() +
#   theme(axis.text.x = element_text(angle = 45, hjust = 1))
# ###data2
# bb <- data2_DB_DOY %>%
#   filter(SiteID == "US-xCL" )
