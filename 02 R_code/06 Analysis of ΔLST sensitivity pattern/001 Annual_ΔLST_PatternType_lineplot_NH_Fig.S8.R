...
#######  0 Data source from the merged raster data file of 02 Europe-America diff_result
#######  Note: The 'Type' is not variable: because the first value of 'type' is the annual average △LST and the second value is 'k'.
#######  Both are values generated from 9 years, and it is used to match the 'type' pattern with different phenological events.

################   01 9-year time series line plot --- Northern Hemisphere  #########################
# Modify 6 groups: 1-SOS 2-MGP 3-GMO 4-GDO 5-MSP 6-EOS
...
df1$average_diff <- as.numeric(as.character(df1$average_diff))    ########################## Note
# df1$group <- "SOS"
...
###  0000. Perform △LST time series for four different regions  
###  Northern Hemisphere NH  6 groups:     ##########
################   01 9-year SOS time series line plot --- Northern Hemisphere  #########################

##  1  2 3 4 5 6   Note modifications                                                     ## Note modification in 2 places
file_list <- list.files(path = "./EA+NA_Results/merge_average_diff_years/merged_average_diff_6/", 
                        pattern = "average_diff_6_\\d{4}\\.tif", full.names = TRUE)
...
for (i in 1:length(file_list)) {
  ...
  common_pixels <- merge(file_df_clean, merged_df1_clean[, c("long", "lat", "type")], by = c("long", "lat"))
...
p1 <- ggplot(final_result, aes(x = year, y = average_diff_mean, group = type, color = type)) +
  ...
  geom_point(size = 5.5) +  # Add points with size 2
  ...
  scale_color_manual(   
    values = c( "#CC3333","#FF9933","#66CCFF", "#006699" ),
  ...
  theme( 
    ...
    axis.title.x = element_text(margin = margin(t = 20)),# Adjust x-axis title distance from x-axis (e.g., set to 20)
    legend.margin = margin(20, 20, 20, 20),  # Adjust legend margins
...
ggsave(filename = "./0.figure/Fig.S8-test-diff&years-NH_6.tiff",     ## Note modification
