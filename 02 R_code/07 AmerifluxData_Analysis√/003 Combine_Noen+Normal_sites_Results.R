...
# 
# # 筛选出至少有三年数据的站点
# sites_with_4_years <- site_year_counts %>%
#   filter(year_count >= 4)
# 
# # 通过筛选得到至少有三年数据的站点的 ID
# valid_sites <- sites_with_4_years$site_id
# 
# # 筛选出有效站点的数据并计算变量的年度平均值，同时保留 site_type 和 year_count
# average_values <- combined_df_1330 %>%
#   filter(site_id %in% valid_sites) %>%
...
# 
# 
# write.csv(average_values, file = "./AmerifluxData_Analysis/1330_Noen+Normal_Results_4y_15.csv", row.names = T)

# 
# 
# ###################################   (-)01-2-选出2013-2021期间至少有5年数据的站点  ####################################
# #23-17=6个站点
# 
# 
# # 统计每个站点的数据年份数量，并加入 site_type
...
#   filter(year_count >= 5)
# 
# # 通过筛选得到至少有5年数据的站点的 ID
# valid_sites <- sites_with_5_years$site_id
# 
# # 筛选出有效站点的数据并计算变量的年度平均值，同时保留 site_type 和 year_count
# average_values <- combined_df_1330 %>%
#   filter(site_id %in% valid_sites) %>%
...
# 
# 
# write.csv(average_values, file = "./AmerifluxData_Analysis/1330_Noen+Normal_Results_5y_6.csv", row.names = T)
# 
# 

# 
# 
# ##################################   02  Test-13：30与 14：30的比较  ############################################
# ########### 13:30
# df1_noen <- read.csv("./AmerifluxData_Analysis/Noen_results.csv")
# df2_norm <-  read.csv("./AmerifluxData_Analysis/Normal_results.csv")
# ########### 14:40
# df11_noen_1430 <- read.csv("./AmerifluxData_Analysis/1430Noen_results.csv")
# df11_norm_1430 <-  read.csv("./AmerifluxData_Analysis/1430Normal_results.csv")
# 
# ########### 去掉SOS、EOS后存在空值的行
# df1_noen <- df1_noen %>%
#   filter(!is.na(average_diff_21) & 
...
#   filter(!is.na(average_diff_21) & 
#            !is.na(average_diff_26))
# df11_noen_1430 <- df11_noen_1430 %>%
#   filter(!is.na(average_diff_21) & 
#            !is.na(average_diff_26))
# df11_norm_1430 <- df11_norm_1430 %>%
#   filter(!is.na(average_diff_21) & 
#            !is.na(average_diff_26))
# #如果要比较两个时间的数据框，那他们有的范围应为一致的，所以去掉不同的行进行分析
# # 找到 df1_noen 中的所有站点和年份组合
# unique_noen <- unique(df1_noen[c("site_id", "Year")])
# 
# # 找到 df11_noen 中的所有站点和年份组合
# unique_noen_1430 <- unique(df11_noen_1430[c("site_id", "Year")])
# 
# # 找到 df1_noen 中的多余行
# extra_rows <- setdiff(unique_noen, unique_noen_1430)
# 
# # 找到多余行对应的完整数据
# rows_to_remove <- df1_noen[df1_noen$site_id %in% extra_rows$site_id & df1_noen$Year %in% extra_rows$Year, ]
# 
# # 打印多余行
# print(rows_to_remove)
# 
# # 从 df1_noen 中删除多余行
# df1_noen_cleaned <- df1_noen[!(df1_noen$site_id %in% rows_to_remove$site_id & df1_noen$Year %in% rows_to_remove$Year), ]
# 
# # 查看清理后的结果
# head(df1_noen_cleaned)
...
# Count the number of years for each site
site_year_counts <- df1_noen %>%
  group_by(site_id) %>%
  summarise(year_count = n_distinct(Year))

# Filter to get sites with at least three years of data
sites_with_three_years <- site_year_counts %>%
  filter(year_count >= 3)

# Count the number of unique sites with at least three years of data
num_sites_with_three_years <- nrow(sites_with_three_years)

# Output the result
cat("The number of sites with data for at least three years is:", num_sites_with_three_years, "\n")

# Count the number of years for each site
site_year_counts <- df2_norm %>%
  group_by(site_id) %>%
  summarise(year_count = n_distinct(Year))
