...
######  测试： 14:30时刻下(更暖) 植被的降温效应是否比13:30的强？
###################################   01 站点信息整合,筛选出DB植被站点  #################################33###



file1 <- "0.download/AmerifluxData/LST_Ameriflux_Filtered.csv/LST_Ameriflux_Filtered.csv"
file2 <- "0.download/AmerifluxData/TransitionDates.csv"
...
data1_DB_selected <- data1_DB %>%
  filter(
    substr(as.character(TIMESTAMP_START), nchar(as.character(TIMESTAMP_START)) - 3, nchar(as.character(TIMESTAMP_START))) == "1430"
  ) %>%
  # 提取年份、日期和 DOY
  mutate(
...
write.csv(data1_DB_selected, "./AmerifluxData_Analysis/1430data1_DB_LST_selected_sum.csv", row.names = FALSE)

# 保存 tau 列非空值部分到指定路径
data1_DB_selected_noen <- data1_DB_selected %>% filter(!is.na(tau))
write.csv(data1_DB_selected_noen, "./AmerifluxData_Analysis/1430data1_DB_LST_selected_noen.csv", row.names = FALSE)

# 保存 tau 列为空值的部分到指定路径
data1_DB_selected_normal <- data1_DB_selected %>% filter(is.na(tau))
write.csv(data1_DB_selected_normal, "./AmerifluxData_Analysis/1430data1_DB_LST_selected_normal.csv", row.names = FALSE)
...
df1_LST_noen <- read.csv("./AmerifluxData_Analysis/1430data1_DB_LST_selected_noen.csv")
...
for (site_id in site_ids) {
  ...
  for (year in years) {
    ...
    if (all(is.na(df2$LST))) {
      ...
      df1_LST_noen_site_year$LST_diff <- df1_LST_noen_site_year$fit_2_value - df1_LST_noen_site_year$LST
...
write.csv(results, file = "./AmerifluxData_Analysis/1430Noen_results.csv", row.names = FALSE)
...
df1_LST_norm <- read.csv("./AmerifluxData_Analysis/1430data1_DB_LST_selected_normal.csv")
df2_PHE_DOY <-  read.csv("./AmerifluxData_Analysis/data2_DB_DOY_selected_sum.csv")
...
for (site_id in site_ids) {
  ...
  for (year in years) {
    ...
    if (all(is.na(df2$LST))) {
      ...
      df1_LST_norm_site_year$LST_diff <- df1_LST_norm_site_year$fit_2_value - df1_LST_norm_site_year$LST
      RMSE_2<- round(sqrt(mean((df2$LST- predict(atc,df2))^2, na.rm = TRUE)), 2)
      R_2 <- round(cor(predict(atc,df2), df2$LST, use = "complete.obs"), 2)
...
write.csv(results, file = "./AmerifluxData_Analysis/1430Normal_results.csv", row.names = FALSE)
